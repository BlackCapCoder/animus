  ...
  if (Async1MSDelay()) // should run once every 1 to 1.5ms
  {
    ...
    for (byte i = 0; i < Global.ROW; i++) // this is placed inside the 1ms delay to slow it down a bit
    {
      for (byte j = 0; j < Global.COL; j++)
      {
        ...
        {
          if (Global.KeyStateCoolDown[j][i] == 255 - Global.RefreshDelayUp) // if key was previously held down
          {
            ReleaseKey(PersMem.GetKeyData(j, i, Global.LayerState[j][i]), PersMem.GetKeyType(j, i, Global.LayerState[j][i]));
            Global.KeyStateCoolDown[j][i] = Global.RefreshDelayDown;
            Global.numKeysDown--;
          }
        }
      }
    ...
    for (byte i = 0; i < Global.ROW; i++)
    {
      for (byte j = 0; j < Global.COL; j++)
      {
        if (Global.KeyStateCoolDown[j][i] > 0 && Global.KeyStateCoolDown[j][i] != 255 - Global.RefreshDelayUp)
        {
...
void CAnimus::MillisLoop()
{
  ...
  {
    // Serial.println(CurrentMillis - PreviousMillis); // 1, 5-7
    PreviousMillis = CurrentMillis;
    ReadyMillis = true;
    RefreshCounter++;
    if (RefreshCounter >= Global.RefreshDelayDown)
    {
      RefreshCounter = 0;
    }
  }
